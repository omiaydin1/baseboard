...
// ---------------------------------------------------------------------------
// Image content screening — NSFW.js + OCR keyword scan
//
// Two independent checks run in parallel on every new image upload:
//   1. NSFW.js  — TensorFlow.js model classifies the image for porn/hentai.
//   2. OCR      — Tesseract.js extracts visible text; scanned against the same
//                 RESTRICTED_KEYWORDS list already used by validateLinkUrl.
//
// Only new uploads are screened — pixels with images/links already on-chain are
// never re-validated. If either check fails the upload is blocked before the
// wallet is ever invoked, with a clear error message.
// ---------------------------------------------------------------------------
...
let _nsfwLoadPromise: Promise<nsfwjs.NSFWJS> | null = null;

async function getNsfwModel(): Promise<nsfwjs.NSFWJS> {
  if (_nsfwModel) return _nsfwModel;
  if (_nsfwLoadPromise) return _nsfwLoadPromise;
  _nsfwLoadPromise = nsfwjs.load();
  _nsfwModel = await _nsfwLoadPromise;
  return _nsfwModel;
}

let _tesseractWorker: Tesseract.Worker | null = null;
let _tesseractInitPromise: Promise<Tesseract.Worker> | null = null;

async function getTesseractWorker(): Promise<Tesseract.Worker> {
  if (_tesseractWorker) return _tesseractWorker;
  if (_tesseractInitPromise) return _tesseractInitPromise;
  _tesseractInitPromise = Tesseract.createWorker("eng");
  _tesseractWorker = await _tesseractInitPromise;
  return _tesseractWorker;
}

/**
 * Classify an image file with NSFW.js. Returns `{ blocked: true }` when the
 * model predicts "Porn" or "Hentai" with probability > 0.7.
 * Throws on load/classification failure — the caller must treat a thrown error
 * as "cannot verify, do not proceed" (fail-closed), not silently skip.
 */
export async function classifyImageNsfw(
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      img.onload = () => resolve();
      img.onerror = () => reject(new Error("Could not decode image for NSFW scan"));
      img.src = url;
    });
...
/**
 * Extract visible English text from an image using Tesseract.js OCR.
 * Throws on failure — caller must treat errors as fail-closed.
 */
...
/**
 * Screen an image file against RESTRICTED_KEYWORDS by OCR'ing any visible
 * text. Returns `{ blocked: true }` if any keyword or pattern matches.
 */
export async function screenImageText(
  ...
  if (
    RESTRICTED_KEYWORDS.keywords.some((kw) => text.includes(kw))
  ) {
    return { blocked: true, reason: "restricted_content" };
  }
  if (
    RESTRICTED_KEYWORDS.patterns.some((re) => re.test(text))
  ) {
    return { blocked: true, reason: "restricted_content" };
  }
  return { blocked: false };
