...
async function main() {
  ...
  // Single transaction: crash ortasında kalma yok, ya hep ya hiç
  ...
  try {
    for (let start = fromBlock; start <= latest; start += LOG_CHUNK + 1) {
      ...
      try {
        ...
        for (const log of logs) {
          ...
          await insertPurchase(tx, blockNumber, txHash, buyer, count, now);
          totalEvents++;
        }
    ...
    if (seenPlotIds.size > 0) {
      ...
      // Resolve basenames for unique owners
      if (uniqueOwners.size > 0) {
        const existingMap = await getBasenames(tx, Array.from(uniqueOwners));
        ...
        for (let i = 0; i < toResolve.length; i += NAME_CHUNK) {
          ...
          for (const r of results) {
            if (r.status === "fulfilled") {
              await upsertBasename(tx, r.value.addr, r.value.name, now);
    ...
    await setLastScannedBlock(tx, latest);
    await tx.commit();
